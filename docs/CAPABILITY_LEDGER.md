...
| **Defensive line height (headline validated result)** | **Validated vs FIFA — restated 2026-07-14** | **raw 16.4 m → de-biased ~7.2 m held-out** (the previously quoted 5.5 m was in-sample: the de-bias slope was fit on all 5 processed matches INCLUDING the 3 FIFA validation matches — contamination caught by external-audit verification, measured at +1.7 m. Quote 7.2 m, or label 5.5 m explicitly as in-sample. LOMO slope range -5.9..-5.0 is stable; the club-only clean slope -6.31 is domain-mismatched to WC broadcasts. Shipped constant unchanged pending a clean WC-broadcast fit match.) |
...
| **Defensive line height** | **Validated vs FIFA (held-out)** | visibility-de-biased; see the headline row above (16.4 m → ~7.2 m held-out; 5.5 m was in-sample) |
